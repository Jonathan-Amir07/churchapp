  ...
  return (
    <PageTransition className="space-y-6 pb-20 md:pb-0">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold text-primary font-display-lg-mobile md:font-display-lg">الفصول الدراسية</h1>
          <p className="font-body-md text-on-surface-variant mt-2 text-lg">إدارة الفصول الخاصة بك.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {stats?.classes?.length > 0 ? (
            stats.classes?.map((c: any) => (
              ...
              >
                ...
                <div className="flex justify-between text-sm text-on-surface-variant mt-4 pt-4 border-t border-outline-variant/50">
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">groups</span> {c._count?.members || 0} مخدوم</span>
                  <span className="flex items-center gap-1 text-primary font-bold">إدارة <span className="material-symbols-outlined text-[16px]">arrow_forward</span></span>
                </div>
              </Link>
            ))
          ) : (
            <div className="col-span-full bg-surface-container-lowest rounded-xl border border-outline-variant p-8 text-center text-on-surface-variant">
              <span className="material-symbols-outlined text-4xl mb-2 text-outline">school</span>
