   ...
   async function fetchTasks() {
     try {
       ...
       const mapped = tasksData?.map((t: any) => ({
         id: t.id,
         titleEn: t.taskTitleEn || t.titleEn || t.lesson?.title || 'Task',
         titleAr: t.taskTitleAr || t.titleAr || t.lesson?.title || 'مهمة',
         points: t.pointsReward || 30,
         status: t.submissions && t.submissions.length > 0 ? t.submissions[0].status : 'not-started',
         instructionsEn: t.description || 'Complete the assignment as described by your instructor.',
         instructionsAr: 'أكمل الواجب كما وصفه لك الخادم.',
         submittedFile: t.submissions && t.submissions.length > 0 && t.submissions[0].attachmentUrl ? {
            name: t.submissions[0].attachmentUrl.split('/').pop(),
            size: 'Unknown',
            type: 'application/pdf',
            url: t.submissions[0].attachmentUrl
         } : undefined,
         feedbackEn: t.submissions && t.submissions.length > 0 ? t.submissions[0].feedback : undefined,
         feedbackAr: t.submissions && t.submissions.length > 0 ? t.submissions[0].feedback : undefined,
       }));
