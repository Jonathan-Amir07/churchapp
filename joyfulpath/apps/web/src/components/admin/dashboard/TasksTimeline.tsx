...
export function TasksTimeline() {
  ...
  return (
    <Card className="border-secondary/20 shadow-sm bg-surface-container-lowest">
      ...
      <CardContent>
        <div className="space-y-4 relative before:absolute before:inset-y-0 before:end-2 before:w-0.5 before:bg-outline-variant/30">
          {tasks?.map((task, i) => (
            <div key={i} className="relative pe-6">
              <span className={`absolute end-[3px] top-1.5 w-2 h-2 rounded-full border-2 border-surface-container-lowest ${task.status === 'urgent' ? 'bg-red-500 ring-2 ring-red-200' : 'bg-primary'}`}></span>
              <div>
